...
# ${PFX}objcopy --add-section .patch=patch.text \
#               --set-section-flags .patch=code,alloc,load,readonly,data,contents \
#               --change-section-address .patch=0x1770  target target_patch && mv target_patch target && rm target.text
# ${PFX}objcopy --add-symbol __patch=".patch:0,function" target target_patch && mv target_patch target
rm patch.text
...
# ${PFX}objcopy --change-section-address .fini+$PATCH_TEXT_SZ target target_patch && mv target_patch target
...
# printf %x $((0x1060+0x142+16)) # text addr + text size + text align
